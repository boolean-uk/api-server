export default [
    {
        'id': 1,
        'title': 'Paris Street; Rainy Day',
        'artist': 'Gustave Caillebotte',
        'imageURL': '/images/paris-street-rainy-day.jpg',
        'publicationHistory': [
            'Catalogue de la 3e exposition de peinture, exh. cat. (E. Capiomont et V. Renault, 1877), p. 3, cat. 1.',
            'Léon Mancino, “La descente de la courtille,” L’art 9 (Apr. 1877), p. 70.',
            'Anonymous [possibly Gaston Vassy], “La journée à Paris: L’exposition des impressionnalistes,” L’événement, Apr. 6, 1877, p. 2.'
        ]
    },
    {
        'id': 2,
        'title': 'Self-Portrait',
        'artist': 'Vincent van Gogh',
        'imageURL': '/images/self-portrait.jpg',
        'publicationHistory': [
            'J[acob]-B[aart] de la Faille, L’œuvre de Vincent van Gogh, Catalogue raisonné vol. 1-2 (Paris: G. Van Oest, 1928), vol. 1, cat. no. 345; vol. 2, fig. 345.',
            'Martha Davidson, “Impressionist and Later Portraits,” Art News 36, no. 23 (1938): 24, 10 (ill.).',
            'Jack Levin, “Homage to Vincent,” Art News 17, no. 8 (1948): 26 (ill.).'
        ]
    },
    {
        'id': 3,
        'title': 'Bathers by a River',
        'artist': 'Henri Matisse',
        'imageURL': '/images/bathers-by-a-river.jpg',
        'publicationHistory': [
            'Charles Estienne, “Des tendances de la peinture moderne: Entretien avec M. Henri-Matisse,” Les nouvelles 2, no. 106 (Apr. 12, 1909), p. 4.',
            'Alvin Langdon Coburn, Men of Mark (Duckworth and Co./Mitchell Kennerly, 1913), pl. 33 (detail).',
            'Ernst Goldschmidt, “Strejftog i Kunsten: Henri Matisse,” Politiken, Jan. 5, 1913, p. 13.'
        ]
    },
    {
        'id': 4,
        'title': 'Nighthawks',
        'artist': 'Edward Hopper',
        'imageURL': '/images/nighthawks.jpg',
        'publicationHistory': [
            'Gail Levin, “Edward Hopper’s Nighthawks, Surrealism, and the War,” Art Institute of Chicago Museum Studies 22, 2 (1996), 180–95, fig. 1.',
            'The Art Institute of Chicago: Twentieth–Century Painting and Sculpture, selected by James N. Wood and Teri J. Edelstein (Chicago: Art Institute of Chicago, 1996), 85, ill.',
            'Judith A. Barter et al., American Modernism at the Art Institute of Chicago, From World War I to 1955 (Chicago: Art Institute of Chicago/Yale University Press, 2009), no. 133.'
        ]
    },
    {
        'id': 5,
        'title': 'Arrival of the Normandy Train, Gare Saint-Lazare',
        'artist': 'Claude Monet',
        'imageURL': '/images/arrival-of-the-normandy-train.jpg',
        'publicationHistory': [
            'Catalogue de la 3e exposition de peinture, exh. cat. (E. Capiomont et V. Renault, 1877), p. 9, cat. 97.',
            'A. P[othey], “Beaux arts,” Le petit parisien, Apr. 7, 1877, p. 2.',
            '“Exposition des impressionnistes: 6, rue Le Peletier; 6,” La petite république française Apr. 10, 1877, p. 2. Reprinted in Ruth Berson, ed., The New Painting: Impressionism, 1874–1886, Documentation, vol. 1, Reviews(Fine Arts Museums of San Francisco/University of Washington Press, 1996), p. 176.'
        ]
    },
    {
        'id': 6,
        'title': 'Improvisation No. 30 (Cannons)',
        'artist': 'Vasily Kandinsky',
        'imageURL': '/images/improvisation-no-30-cannons.jpg',
        'publicationHistory': [
            'Kandinsky, 1901-1913 (Berlin: Der Sturm, 1913), p. 8 (ill.).',
            'The London Salon of the Allied Artists’ Association, Ltd., exh. cat. (London: Allied Artists’ Association, Ltd., 1913), p. 15, cat. 286.',
            'Eddy, Arthur Jerome, Cubists and Post-Impressionism (Chicago: A. C. McClurg & Co., 1914), pp. 124-26, opp. p. 124 (color ill.).'
        ]
    },
    {
        'id': 7,
        'title': 'The Herring Net',
        'artist': 'Winslow Homer',
        'imageURL': '/images/the-herring-net.jpg',
        'publicationHistory': [
            '“Fine Arts: The Autumn Academy,” Nation 41 (Dec. 17, 1885), 516–17.',
            'W.A. Coffin, “A Painter of the Sea,” Century Magazine (Sept. 1899).',
            'R. Hitchcock, The Art of the World: The World’s Columbian Exposition (New York: D. Appleton and Company, 1895) Section 4, ill. p. 143.'
        ]
    }
]